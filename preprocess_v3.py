...
class PreprocessDataset(Dataset):
    def __init__(self, data, tokenizer):
        ...
        result_matrix = [
            [0] + row + [tokenizer.eos_token_id] for row in tgt_id.input_ids
        ]
        self.data = {
...
if __name__ == "__main__":
    ...
    mono_file = "news.2023.de.shuffled.deduped"
    # debug_cnt=0
    ...
    if mono and clm_mode and input == "train":
        ...
        with open(mono_file) as f:
            ...
            for line in tqdm(f, desc="assist procedure"):
                ...
                line = line.strip()
                assist = tokenizer(
                    line, add_special_tokens=False, return_attention_mask=False
                )
    ...
    pickle.dump(
        real_total_dict,
        open(output_dir + "_supervised", "wb"),
        protocol=5,
    )
    if clm_mode:
        pickle.dump(
            real_total_clm_dict,
            open(output_dir + "_clm", "wb"),
            protocol=5,
        )
